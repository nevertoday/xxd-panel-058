<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 058 project banner" width="1200"></p>
...
<table>
  <tr>
    <td width="33.33%"><a href="https://x.com/xiaoxiaodong01/status/2092176222445035612"><img src="./assets/examples/sample-01.jpg" alt="XXD Panel 058 예시 1"></a></td>
    <td width="33.33%"><a href="https://x.com/xiaoxiaodong01/status/2092176222445035612"><img src="./assets/examples/sample-02.jpg" alt="XXD Panel 058 예시 2"></a></td>
    <td width="33.33%"><a href="https://x.com/xiaoxiaodong01/status/2092176222445035612"><img src="./assets/examples/sample-03.jpg" alt="XXD Panel 058 예시 3"></a></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 058 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 058 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 058 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 058 예시 8"></td>
  </tr>
</table>

## 추가 3:4 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 058의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 058 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 058 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 058 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 058 추가 상하 예시 4"></td>
  </tr>
</table>

> [샤오샤오둥（@xiaoxiaodong01）](https://x.com/xiaoxiaodong01/status/2092176222445035612) · 2026년 8월 25일<br>
> GPT2 × 기하 × 추상 × 영혼 × 미학 프롬프트 × VOL.058

<p align="center"><a href="https://x.com/xiaoxiaodong01/status/2092176222445035612">원문 게시물과 전체 프롬프트 보기 →</a></p>

이 예시는 058의 미학적 결과만 보여 줍니다. 예시의 주제, 구성, 색상, 문구, 이전 캔버스 비율은 새 작업의 참고나 기본값이 되지 않습니다.

숨은 뜻 읽기 · 기하 미니멀리즘 · 개념 풍경 · 부드러운 수공 질감 · 옅은 여백

사진이 직접 말하지 않는 방향, 거리, 기다림, 떠남, 가까워짐을 읽고 그 관계를 길, 지형, 건축적 덩어리, 상징적 형태로 번역합니다. 이성적인 비례와 축선, 공간 질서가 부드러운 수공 질감을 붙들어 평범한 사진에서 고요하고 낭만적인 우화를 끌어냅니다.
...
**Panel 058**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진의 숨은 뜻을 읽고 엄격한 기하로 시적인 개념 풍경을 재구성**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
> XXD Panel 058으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 058이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진의 숨은 뜻을 읽고 엄격한 기하로 시적인 개념 풍경을 재구성 |
| **한눈에 알아볼 특징** | 숨은 뜻 읽기 · 기하 미니멀리즘 · 개념 풍경 · 부드러운 수공 질감 · 옅은 여백 |
...
보이는 대상뿐 아니라 사진이 무엇을 암시하고 상징하며 연상시키는지 읽습니다. 의미 있는 주체와 관계를 남기고 설명적인 세부를 덜어 낸 뒤, 소수의 형태와 길, 거리, 공간 층으로 원본과 같은 뿌리를 가진 개념 장면을 만듭니다.

```text
정체성·윤곽·자세 고정 → 숨은 뜻·감정·함의 읽기 → 불필요한 현실 세부 삭제 → 중심 주제와 상징 관계 추출 → 형태·길·축선·원근·거리로 개념 풍경 구성 → 옅은 여백과 부드러운 수공 질감으로 시성 유지 → 원본과 연결된 최소한의 글자 통합
```

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- 주체와 관계는 산, 건축, 도로, 플랫폼, 돔, 원반 등 소수의 상징 형태로 바뀌며 현실 장면 전체를 베끼지 않습니다.
- 비례, 축선, 원근, 경로 유도, 면의 관계와 공간 균형이 엄격한 이성적 질서를 만듭니다.
- 옅은 핑크 블루, 안개빛 파랑, 하늘색, 아이보리, 크림과 소량의 블러시가 흰색에 가까운 여백 속에서 투명한 층을 만듭니다.
- 은은한 종이·분말·판화 질감이 기하 평면에 부드러운 손맛을 남기고 스티커, 사실 묘사, 매끈한 3D를 피합니다.
- 글자는 독립 출판물의 표제나 관찰 메모처럼 축선, 경로, 가장자리 또는 주체 주변에 가볍게 놓입니다.

미적 제약과 금지 항목은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있으며 Skill과 런타임 어댑터는 이번 출력 변수만 처리합니다. [Skill](SKILL.md) · [영문 런타임 어댑터](references/xxd-panel-058-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 058의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-058 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-058 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-058 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-058 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-058 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-058 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 058 적용, 공통 설정은 한 번만 확정
/xxd-panel-058 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-058`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-058-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-058-prompt.zh-CN.md)
