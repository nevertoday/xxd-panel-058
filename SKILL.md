...
Create fresh PNG artwork with `references/058-source.md` as the **sole creative and aesthetic authority**. This Skill is a runtime shell: it resolves delivery variables, appends them to the untouched source-brief body, executes bitmap generation, and verifies the result. It must not write a second art direction.

## Authority boundary

1. Read `references/058-source.md` completely immediately before building every generation request.
...
```text
/xxd-panel-058 <source-or-directory> --mode top-bottom,design-only \
...
1. 上下对照（原图在上，058 设计在下）
2. 左右对照（原图在左，058 设计在右）
...
```text
[VERBATIM SOURCE-BRIEF BODY FROM references/058-source.md,
...
```text
~/Desktop/xxd/xxd-panel-058/<fresh-task>/
...
- the result follows `references/058-source.md`, especially its own colour, material, composition, whitespace, and typography requirements, without an outer Skill palette or added art direction;
...
- `references/xxd-panel-058-prompt.zh-CN.md`
- `references/xxd-panel-058-prompt.en.md`

These files document the same minimal adapter. They never replace `references/058-source.md`.
